...
本轮将 GUI 收口到 `0.4.0-prebeta`：本地 SVG IconRegistry、TitleBar/Design Tokens/GlassDialog、真实 Hermes/Claude Code/Codex Detection、严格 cc-connect Runtime Readiness、Binding 与 Chat Live Health 分离、Hermes Telegram Native Configuration、六链路 Live E2E 用户确认、Dashboard/Diagnostics/全局刷新和 1280×720 原生布局回归已完成。本地全量为 264 passed、2 skipped，Ruff、format、`mypy control_plane` 112 files 与契约验证通过；这不是用户 live 证据。
...
下一阶段为用户用 `0.4.0-prebeta` candidate 做新 GUI Telegram 与 Hermes Native Telegram 真实验收，再做 Windows 10 x64 普通用户实机验证；通过后进入 Installer、卸载、快捷方式、Release Asset 与签名准备。历史 2026-08-07 六链路只保留为旧入口 `LIVE_VERIFIED`。
