...
# CREATE_NO_WINDOW,避免 subprocess 弹出可见黑窗(仅 Windows)
...
def _hermes_exe_candidate() -> str | None:
...
def _cc_connect_exe_candidate() -> str | None:
...
class WindowsSystemDiscoveryAdapter(DiscoveryAdapter):
    ...
    def capabilities(self) -> list[Capability]:
        return [_read_only_capability("system.discover.v1")]


class HermesDiscoveryAdapter(DiscoveryAdapter):
...
class CcConnectDiscoveryAdapter(DiscoveryAdapter):
...
class ClaudeCodeDiscoveryAdapter(DiscoveryAdapter):
...
class CodexDiscoveryAdapter(DiscoveryAdapter):
